...
COST_TYPE HeuristicManager::computeInformedHeuristics(
    ...
    // TODO: add timer
    int num_of_agents=graph->get_num_agents();
...
void HeuristicManager::buildCardinalConflictGraph(
    ...
    int num_of_agents=graph->get_num_agents();
    int num_of_states=graph->get_num_states();

    // get all conflicts
    auto last_state_idxs=*(graph->accum_state_cnts_end);
    ...
    if (fast_approximate) {
        auto & agent_state_cnts=*(graph->state_cnts);
        // TODO(rivers): why would we iterate through all states?
        for (int i=0;i<num_of_states;++i) {
            COST_TYPE i_time=longest_paths[i];
            int agent_i, state_i;
            tie(agent_i, state_i) = graph->get_agent_state_id(i);
            auto & in_state_idxs = graph->switchable_type2_edges->get_out_neighbor_global_ids(i);
            for (auto j: in_state_idxs) {
                ...
                if (diff>=0 && back_diff>=0) {
                    int agent_j, state_j;
                    tie(agent_j, state_j) = graph->get_agent_state_id(j);
