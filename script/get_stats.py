...
result_folder="output/2024_03_10_22_07_32_exp"
...
result_headers=["algo","branch_order","random_seed","status","search_time","total_time","ori_total_cost","total_cost","ori_trunc_cost","trunc_cost",
    ...
    for sit_idx in range(num_sits):
        for algo in algos:
            for branch_order in branch_orders:
                trial_name="map_{}_ins_{}_an_{}_sit_{}_algo_{}_{}".format(map_name,instance_idx,agent_num,sit_idx,algo,branch_order)
...
summary=df.groupby(by=["map_name","agent_num","algo","branch_order"]).mean().reset_index()
print(summary)
summary.to_csv(stat_summary_ofp)

print(df.groupby(by=["map_name","agent_num","algo","branch_order"])["total_time"].quantile([0.4,0.6]))