...
class ConstrainedAgentsLoader {
 public:
  int num_of_agents;
  int max_k = 0;
  vector<Location> initial_locations;
...
//   AgentsLoader(const std::string fname, const MapLoader &ml,int max_k, bool diff_k, int agentsNum);
  ConstrainedAgentsLoader();
//   AgentsLoader(int number_of_agent,int max_k);
//   AgentsLoader(p::object agents, int max_k ,bool diff_k);

  ConstrainedAgentsLoader(ConstrainedMapLoader & constrained_map_loader, int max_k, bool diff_k);
